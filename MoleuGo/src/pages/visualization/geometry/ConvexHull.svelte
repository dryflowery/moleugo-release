<script>
    ...
    const drawConvexHull = async (myAsync) => {
        animationStep = [-1, animationQuery.length - 1];
    ...
    const playConvexHullAnimation = async (i, myAsync) => {
        // point 속성 변경 함수
        const changePointColor = (pointElements, nodeNum, border, color, opacity) => {
            pointElements[pointsInfo[nodeNum].pointElementsIdx].style.border = border;
            pointElements[pointsInfo[nodeNum].pointElementsIdx].style.color = color;
            pointElements[pointsInfo[nodeNum].pointElementsIdx].style.opacity = opacity; 
        };
        ...
        if(i == -1) { // 처음 한 번만 실행됨
        ...
        else if(i == 1) { // 1. sort by position + set points[0]
            // fromBtn 작성하기@@@@@@@@@@@@@@@@@@@@@
            ...
            await delay(50); // 배속 빨라지면 grid가 작아진 상태에서 firstRect가 선택되기 때문에 delay 필요
            ...
            while(true) {
                if(((myAsync + 1) != asyncCnt) || (await rowAnimate(dist++) === true)) break;
            }

            // 세로줄 그리기
            dist = 0;

            const colAnimate = async (dist) => {
                await waitPause();
            ...
            while(true) {
                if(((myAsync + 1) != asyncCnt) || (await colAnimate(dist++) === true)) break;
            }
            ...
            const animateSemiCircle = async () => {
                ...
                while(nodeNum < pointsInfo.length) {
                    if(await isPositionOnLine(endX, endY) === false) {
                        break;
                    }
                }

                if(nodeNum == pointsInfo.length) {
                    return false;
                }
            ...
            // 노드 중심에 선이 닿으면 해당 노드에 번호 부여
            const isPositionOnLine = (endX, endY) => {
                let rect = pointElements[pointsInfo[nodeNum].pointElementsIdx].getBoundingClientRect();
                ...
                if(Math.abs((endX - startX) * (curY - startY) - (endY - startY) * (curX - startX)) <= 3000) {
                    pointsNum[pointsInfo[nodeNum].pointsNumIdx] = nodeNum++;
                    return true;
                }
            ...
            while(angle <= 180) {
                if(((myAsync + 1) != asyncCnt) || (await animateSemiCircle() === false)) break;
            }

            await delay(2000 * (1 / animationSpeed));

            // 반시계로 정렬 후 svg path 전부 삭제
